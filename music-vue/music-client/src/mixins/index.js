import {likeSongOfName,getByUserId} from "../api/index";
  ...
  methods: {
    ...
    async getSong(){
      if(!this.$route.query.keywords){
        ...
        const {data:res} = await likeSongOfName(this.$route.query.keywords)
        if(res.data.songList.length == 0){
          this.$message.error("没有符合您条件的歌曲")
          this.$store.commit('setListOfSongs',[])
        }else{
          this.$store.commit('setListOfSongs',res.data.songList)
        }
      }
