...
import {getConsumerById, getByUserId,getSongByKId} from '../api/index';
...
export default {
  ...
  methods:{
    ...
    getCollection(userId){
      getByUserId(userId)
        .then(res =>{
          this.collection = res.data.data.collectList;
          //通过歌曲id获取歌曲信息
    ...
    getSongsOfIds(id){
      getSongByKId(id)
        .then(res =>{
          this.collectList.push(res.data.data.song);
        })
        .catch(err => {
          console.log(err);
        })
