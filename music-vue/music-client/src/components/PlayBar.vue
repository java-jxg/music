...
<script>
import {download, getByUserId} from '../api/index';
  ...
  methods:{
    ...
    toplay: function(url){
      if(url && url != this.url){
        ...
        this.$store.commit('setLyric',this.parseLyric(this.parseLyric(this.listOfSongs[this.listIndex].lyric)));
    ...
    async collection(){
      if(this.loginIn){
        ...
        this.collectForm.type = 0
        this.collectForm.songId = this.id
        const{ data:res } = await addCollect(this.collectForm)
