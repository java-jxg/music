...
<script>
import {getAllConsumer} from "../api/consumer";
import {getAllSinger} from "../api/signer";
...
export default {
  ...
  methods:{
    async getConsumer(){
      ...
      this.consumerCount = res.data.consumerList.length
      this.consumer = res.data.consumerList
      this.consumerSex.rows[0]['总数'] = this.setSex(1,this.consumer);
      this.consumerSex.rows[1]['总数'] = this.setSex(0,this.consumer);
    },
    async getSong(){
      const{ data:res } = await getAllSong()
      this.songCount = res.data.songList.length
    },
    async getSinger(){
      const{ data:res } = await getAllSinger()
      this.singerCount = res.data.singerList.length

      this.singerSex.rows[0]['总数'] = this.setSex(0,res.data.singerList);
      this.singerSex.rows[1]['总数'] = this.setSex(1,res.data.singerList);
      this.singerSex.rows[2]['总数'] = this.setSex(2,res.data.singerList);
      this.singerSex.rows[3]['总数'] = this.setSex(3,res.data.singerList);
      for(let item of res.data.singerList){
        this.getByCountry(item.location);
      }
    },
    async getSongList(){
      const{ data:res } = await getAllSongList()
      this.songListCount = res.data.songLists.length
      for(let item of res.data.songLists){
